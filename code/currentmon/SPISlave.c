...
static struct commandJumpTableEntry const handlerTable[] PROGMEM = {
	{	// Command ReadCurrentValues (ID 0x0)
		.masterDataLength = 3,
		.messageLength = 9,
		.handler = (genericFunctionPointer)executeReadCurrentValues
	},
};
...
void spiFieldLengthAssertions(void) {
	uint8_t ReadCurrentValuesAssertion[sizeof(struct cmCmd_ReadCurrentValues) == 9 ? 0 : -1];

	(void)ReadCurrentValuesAssertion;
}
#endif
